...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gjpy.LightScryptN, Gjpy.LightScryptP);
	...
	// signatures too. Regression test for go-japariyen issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogjpyer
	...
	workspace, err := ioutil.TempDir("", "gjpy-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethereum/gjpytest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gjpy and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/japariyen/go-japariyen/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gjpy.aar"), "gjpy.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethereum", "gjpytest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
