...
#
# The cluster contains a bootnode, a gjpy node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gjpy and
# swarm nodes
start_bootnode() {
...
# start_gjpy_node starts a gjpy node with --datadir pointing at <base-dir>/gjpy
# and a single, unlocked account with password "gjpy"
start_gjpy_node() {
  local dir="${base_dir}/gjpy"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gjpy account address"
  fi
  ...
  start_node "gjpy" "${GETH_IP}" "$(which gjpy)" ${args[@]}
}
...
create_account() {
  ...
  gjpy --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
